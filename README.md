...
- [IV. Further development (ex: adding new Business Cases):](#iv-further-development-ex-adding-new-business-cases)
  ...
  - [2. Write your new code](#2-write-your-new-code)
    ...
    - [New plot function](#new-plot-function)
  ...
  - [3. Add references to the new stuff in the old code](#3-add-references-to-the-new-stuff-in-the-old-code)
    - [New Business Case](#new-business-case-1)
    - [New plot function](#new-plot-function-1)
...
| [tests.py](src/tests.py) | Allows accessing functionality defined for the BCA without having to go through the GUI. | 
...
| [popup.py](src/frontend/popup.py) | A file which contains the code for the progress bar which appears as a popup.  | 
...
| [logger.py](src/libs/logger.py) | Sets up the logger which is used throughout the program to maintain a clear backtrace | 
...
| [general_methods](src/methods/general_method.py) | This file defines the Generalized method for calculating the BC  | 

If you write any new methods add them here !

### Modify 

| File Name |  Purpose | 
| -------------- | ----------------------------------------------------- |
| [settings.py](src/modify/settings.py) | This file contains all the freely modifiable settings. |
| [plots.py](src/modify/plots.py) | The functions defining the different plots offered to the user.  | 
| [bca_entrypoint.py](src/modify/bca_entrypoint.py) | This file contains the entrypoint into the BC logic.  | 
| [bca_class.py](src/modify/bca_class.py) | Contains the class which contains all the variables that are used throughout the BC.| 
...
1. *Variables defined before the BC truly begins*:
...
2. *Variables that get defined during the BC*:
- **years_covered**: Amount of years covered by the BC 

3. *Varables defined on a per-scenaio basis that needed to be saved* 
...
So your new file MUST look like this:
```python
def name_of_your_new_bc_method(business_case, scenario_index, debug_mode):
  ...
  result = run_bus_case(
    ...
    business_case.param_df.iloc[scenario_index, x:y] = result  
    # Here x is the position of the first column of the parametric Analysis sheet you have calculated and y is the last one.
    return

def run_bus_case(business_case, var1, var2, ...)
  # Your code:
  ...
...
Of course youc an add more functions and code than just these two, but please be aware that theh only part of your code that will be accessed by the rest of the app is the name_of_your_bc_method function
...
- and you must add the following line near the beginning:
```python
 if business_case.plotting:
        # Create a new popup Tkinter window
        popup = tk.Toplevel()
        popup.title(f"Scenario {scenario_name}: Plot Name")
...
```python
    ...
    log_print(f"PLOT NAMED plot displayed in popup for scenario {scenario_name}.")
    save_figure(fig, "plotname", scenario_name)
    log_print(f"PLOT NAMES plot saved for scenario {scenario_name}.")
...
1. Go to the [settings](src/modify/settings.py) file and add your method to the CHOICE_MATRIX, following the same format as the prior ones. If your BC is for a new case type you can also add an entry there for the case_type. Then add a reference to the function of your method to the METHOD_SET variable in the right position (You can determine the number of your method which corresponds to its position in the METHOD_SET by simply counting left to right, top to bottom, all the methods in the CHOICE_MATRIX (make sure you count the firt entry as the 0th))

And done! Normally your new BC should be perfectly accessible.

2. Remarks: if your new BC requires new inputs, or key variable to be carried over between functions or to the plots, make sure to add your inputs to the Business_Case class defined in the [bca_class](src/modify/bca_class.py) and then have the variable be assigned a value in the setup() function of the class or at the beginning of your BC code.  
