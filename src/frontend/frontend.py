...
class BCA_App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.entries: dict[str, list[ttk.Entry | float | str]] = (
    ...
    def create_entry_widgets(self) -> dict[str, list[ttk.Entry | float | str]]:
        ...
        """
        entries: dict[str, list[ttk.Entry | float | str]] = {
        ...
        if self.debug_mode.get():
            log_print(
                f"Currently in creation of entry dict the created dict is: {entries}"
            )
        return entries
    ...
    def create_entry(self, name: str, position: int) -> ttk.Entry:
        ...
        entry = ttk.Entry(
            self.root,
            validate="key",
            validatecommand=(self.isOkay, "%W", "%P"),
            invalidcommand=(self.correctInput, "%W"),
        )
    ...
    def create_buttons(self) -> None:
        ...
        self.load_vals_button.grid(
            column=1, row=self.amount_widgets, padx=10, pady=20, sticky="nsew"
        )

        self.load_vals_label = ttk.Entry(
            self.root,
            validate="key",
            validatecommand=(self.isOkay, "%W", "%P"),
            invalidcommand=(self.correctInput, "%W"),
        )
        self.load_vals_label.grid(
            column=2, row=self.amount_widgets, padx=10, pady=20, sticky="nsew"
        )
        ...
        case_choice_label.grid(
            column=1, row=self.amount_widgets, pady=20, padx=10, sticky="nsew"
        )
        case_type_combo = ttk.Combobox(
            self.root, textvariable=self.case_type_choice, state="readonly"
        )
        case_type_combo["values"] = list(CHOICE_MATRIX.keys())
        case_type_combo.grid(
            column=2, row=self.amount_widgets, pady=20, padx=10, sticky="nsew"
        )
        ...
        method_choice_label.grid(
            column=1, row=self.amount_widgets, pady=20, padx=10, sticky="nsew"
        )
        self.method_combo = ttk.Combobox(
            self.root, textvariable=self.method_choice, state="readonly"
        )
        ...
        options_button = tk.Button(
            self.root, text="Options", command=self.open_options_menu
        )
        options_button.grid(
            column=1, row=self.amount_widgets, pady=20, padx=10, sticky="nsew"
        )
    ...
    def launch_button(self) -> None:
        ...
        if debug_status:
            for plot in AVAILABLE_PLOTS:
                try:
                    log_print(
                        f"State of plot called: {plot}, {self.selected_plots[plot].get()}"
                    )
                except Exception:
                    log_print(
                        f"An error occured for {plot}, program will keep executing but beware potential undefined behaviour ahead"
                    )
        try:
            self.update_widgets(self.entries)
        except ValueError:
            messagebox.showwarning(
                "Warning!", "Some of the inputs are empty or invalid"
            )
            return
        ...
        clean_input_values: dict[str, Any] = {
            key: value[1] for key, value in self.entries.items()
        }  # Actual type of values[1] is str|float
        ...
        # Label (optional)
        progress_pp = Progress_Popup(
            tk.Label(popup, text="Running simulation..."),
            ttk.Progressbar(popup, orient="horizontal", length=300, mode="determinate"),
        )
    ...
    def no_error_run(
        self,
        selected_file: str,
        excel_output_sheet_name: str,
        debug_status: bool,
        paste_to_excel: bool,
        case_type: int,
        method: int,
        input_values: dict[str, Any],
        selected_plots: dict[str, Any],
        progress_pp: Progress_Popup,
        gen_flag: bool,
        recalc_flag: bool,
    ):
        ...
        try:
            ...
            progress_pp.bar.stop()
            progress_pp.label.config(
                text="Error (ValueError): "
                + str(e)
                + "\n Make sure you selected the right method and named the columns correctly."
            )
            log_print(
                f"Warning (ValueError Raised): {str(e)} \n Make sure you selected the right method and named the columns correctly."
            )
        except Exception as e:
            progress_pp.bar.stop()
            progress_pp.label.config(
    ...
    def update_widgets(self, entries: dict[str, list[ttk.Entry | float | str]]) -> None:
            ...
            if key in STRING_BASED:
                if key == "Scenario(s) (seperate with ',' or write 'All')":
                    if isinstance(entries[key][0], ttk.Entry):
                        ...
                        raise TypeError(
                            f"Warning, Unexpected behavior occured: first value of self.entries isn't an Entry, it currently is {type(entries[key][0])}"
                        )
                else:
                    if isinstance(entries[key][0], ttk.Entry):
                        entry_widget = cast(ttk.Entry, entries[key][0])
        ...
        if self.debug_mode.get():
            log_print(f"Case type is {self.case_type} and method is {self.method}")
        return
    ...
    def validate_input(self, who: str, what: str) -> bool:
        ...
        if what == "":
            return True
        if who in [
            ".!entry" + str(find_index(self.entries, element) + 1)
            for element in STRING_BASED
        ] or (who == ".!entry" + str(len(self.entries) + 1)):
            try:
                input = str(what)
    ...
    def erase_input(self, who: str) -> None:
        ...
        if who in [
            ...
            if who == ".!entry" + str(
                find_index(
                    self.entries, "Scenario(s) (seperate with ',' or write 'All')"
                )
                + 1
            ):
                messagebox.showwarning(
                    "Invalid Entry", "Please enter a letter followed by a number"
                )
            else:
                messagebox.showwarning(
                    "Invalid Entry", "Please enter an actual sheet name"
                )
        else:
    ...
    def open_options_menu(self) -> None:
        ...
        ttk.Label(popup, text="Select which plots").grid(
            column=0, row=0, padx=5, sticky="w"
        )
        ttk.Label(popup, text="you want to compute").grid(
            column=0, row=1, padx=5, sticky="w"
        )

        i: int = 0
        for plot in AVAILABLE_PLOTS:
            ttk.Checkbutton(popup, text=plot, variable=self.selected_plots[plot]).grid(
        ...
        ttk.Checkbutton(
            popup,
            text="Do you wish to automatically paste the result to excel?",
            variable=self.paste_to_excel,
            command=toggle_entry,
        ).grid(column=0, row=i, pady=10, sticky="sw")
        ttk.Label(popup, text="Enter output sheet name:").grid(
            column=0, row=i + 1, padx=5, sticky="w"
        )
        ...
        ttk.Checkbutton(
            popup,
            text="Do you wish to use the general methods?",
            variable=self.use_gen_method,
        ).grid(column=0, row=i, pady=10, sticky="sw")
        i += 1
        ...
        # Debug_button
        ttk.Checkbutton(
            popup, text="Enable Debug mode?", variable=self.debug_mode
        ).grid(column=0, row=i, pady=10, columnspan=2)

        def submit() -> None:
            selected = [plot for plot, var in self.selected_plots.items() if var.get()]
            self.excel_output_sheet_name = excel_output_entry.get()
            messagebox.showinfo(
                "Selected Plots", f"Selected plots: {', '.join(selected)}"
            )
            popup.destroy()
            return

        ttk.Button(popup, text="Confirm", command=submit).grid(
            column=0, row=i + 1, columnspan=2
        )
    ...
    def load_vals(self) -> None:
        if self.load_vals_label.get() != "":
            ...
            try:
                vals = pd.read_excel(
                    self.selected_file,
                    sheet_name=self.load_vals_label.get(),
                    index_col=0,
                    header=None,
                )
                log_print(
                    f"Values found in sheet names {self.load_vals_label.get()}: \n {vals}"
                )
                for key in self.entries:
                    if key not in STRING_BASED:
                        log_print(
                            f"Currently modifying value of {key} and replacing with {vals.loc[key].dropna().iloc[0]}"
                        )
                ...
                log_print(
                    f"Error, Row name mismatch, first error occured at row named: {e}"
                )
                messagebox.showwarning(
                    "Error", f"Row name mismatch, first error occured at row named: {e}"
                )
            except Exception as e:
