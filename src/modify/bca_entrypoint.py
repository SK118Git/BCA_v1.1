...
def run(
    ...
    if not (recalc_flag):
        force_excel_calc(
            file_name
        )  # Force excel to recalculate the sheets of the file, this adds overhead but elimantes many bugs
...
def launch_analysis_new(
    business_case: Business_Case, scenario_index: int, debug_mode: bool, gen_flag: bool
):
    """
