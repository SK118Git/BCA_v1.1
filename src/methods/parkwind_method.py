...
def parkwind_method(business_case, scenario_index):
    ...
    result = run_bus_case(business_case, ppa_price, solar_MWp, bal_per, power_level, storage_time_hr, price_type)

    business_case.param_df.iloc[scenario_index, 7:18] = result
    return 


def run_bus_case(business_case, ppa_price, solar_MWp, bal_per, power_level, storage_time_hr, price_type):
    global cash_flows
    df = business_case.df 

    df['Balancing Prices [Euro/MWh]'] = df["Day-Ahead Prices [Euro/MWh]"] + (df["Imbalance Prices [Euro/MWh]"] - df["Day-Ahead Prices [Euro/MWh]"]) * 0.5
    ...
    #maximum power
    maxpower = business_case.input_values['Export Transmission Capacity'] #transmission power rating

    #%

    # Storage capacity (corrected for settlement period)
    storage_time = (1/settlement_period)*storage_time_hr #(corrected for settlment period)
    ...
    df['Exported Power [MW]'] = np.where(df['Available Power [MW]'] > maxpower, maxpower, df['Available Power [MW]'])
    ...
    df['theor_discharging'] = np.where (df['deltapower'] < 0 , 0 ,
                              np.where (df['Balancing Prices [Euro/MWh]'] > (1.3 * df['Wholesale_Price']) , - power_level , 0 ))
    ...
    for i in range(len(df)):
      ...
      soc_val = np.clip(last_soc_val + max_charge_or_discharge_np[i], a_min = 0, a_max = capacity)
    ...
    df['bal_income'] = ((df['Wholesale_Price']+ green_certificate) * (df['Exported Power [MW]'] * (100-bal_per)/100)* settlement_period) + (bal_per / 100 * df['Exported Power [MW]'] * (df['Balancing Prices [Euro/MWh]']+ green_certificate) * settlement_period)
    ...
    df['storage_income'] = np.where (df['eff_charge_discharge'] == 0, df['bal_income'],  #[A]
                           np.where (df['eff_charge_discharge'] < 0 , df['bal_income'] - df['eff_charge_discharge'] * df['Balancing Prices [Euro/MWh]'] * settlement_period, #[B]
                           np.where (df['deltapower'] < 0           , df['bal_income'],  #[C]       
                           np.where (power_level <= df['bal_power'] , df['bal_income'] - df['eff_charge_discharge'] * df['Balancing Prices [Euro/MWh]'] * settlement_period, #[D]
                           np.where (power_level <= df['Exported Power [MW]'] , df['bal_income'] - df['bal_power']*df['Balancing Prices [Euro/MWh]']*settlement_period - (df['eff_charge_discharge'] - df['bal_power']) * (df['Wholesale_Price'] + df['Balancing Prices [Euro/MWh]'])*settlement_period, #[E]
                           (df['bal_power'] - power_level) * df['Balancing Prices [Euro/MWh]'] * settlement_period #[F]
                            )))))                         
    ...
    storage_total_income = (df['storage_income'].sum() + df['extra_generation_income'].sum())       #Wind + PV + Storage total income
    storage_net_income = storage_total_income - df['baseline_income'].sum()                         #Income only attrubuted to storgae: [A] + [B] + [C]
    
    cash_flows = [-1*Storage_CAPEX] + [(storage_net_income - Storage_OPEX)] * Project_Life
    ...
    result = [
        (df['extra_generation'].sum() + df['Exported Power [MW]'].sum()),       #G: Annual Energy Generated
        ...
        (df['bal_income'].sum() - df['baseline_income'].sum()),                 #L: Revenue (A) - Direct Balancing Market 
        (df['storage_income'].sum() - df['bal_income'].sum()),                  #M: Revenue (B) - Stored Energy to Balancing Market
        df['extra_generation_income'].sum(),                                    #N: Revenue (C) - Extra Generation-Based Income
