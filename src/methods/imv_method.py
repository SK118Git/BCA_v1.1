...
def imv_method(business_case:Business_Case, scenario_index:int):
    ...
    business_case.df['Available Power [MW]'] = num_turbines*business_case.df['Wind Speed [m/s]'].apply(siemens_gamesa_power_curve)
    ...
    #maximum power
    business_case.df['Available Transmission Capacity [MW]'] = transformer_rating - business_case.df['Capacity Constraint [MW]'] #transmission power rating
...
def run_bus_case(business_case:Business_Case, ppa_price, bal_per, price_type, power_level, storage_time_hr, years_covered):
    ...
    result = [
        df['Available Power [MW]'].sum()*settlement_period/years_covered,       #H: Total Energy that could be produced assuming no transmission constraint
        (df['Exported Power [MW]'].sum() + df['extra_generation'].sum())*settlement_period/years_covered,       #G: Total Energy Generated
        df['Net Exported Power_Storage [MW]'].sum()*settlement_period/years_covered,                            #H: Total Energy Exported with Storage
        df['Curtailed Power [MW]'].sum()*settlement_period/years_covered,                                       #I: Total Energy Curtailed Energy 
        ...
        df['end_soc_values'].iloc[-1]/years_covered,                            #Energy Held in storage at the end of the simulation, annualised for year-fraction    
        Storage_CAPEX,                                                          #J: Storage CAPEX
        Storage_OPEX,                                                           #K: Storage OPEX
        df['baseline_income'].sum()/years_covered,                              #L: Baseline Revenue
        (df['bal_income'].sum() - df['baseline_income'].sum())/years_covered,   #M: Revenue (A) - Direct Balancing Market 
        (df['storage_income'].sum() - df['bal_income'].sum())/years_covered,    #N: Revenue (B) - Stored Energy to Balancing Market
        df['extra_generation_income'].sum()/years_covered,                      #O: Revenue (C) - Extra Generation-Based Income
        storage_total_income/years_covered,                                     #P: New Wind Farm Revenue with Storage [Nominal + A+B+C]
